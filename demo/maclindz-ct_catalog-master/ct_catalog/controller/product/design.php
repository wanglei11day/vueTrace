...
require DIR_SYSTEM.'library/aws/aws-autoloader.php';
	class ControllerProductDesign extends Controller {
        public $S3BUCKET = 'upload.creator5.permanent';
		...
		public function index() {
			...
			if (isset($this->request->get['product_id'])) {
				...
				if ($data['customer_creation_id']) {
					...
					$diydesign = $this->model_account_diy_design->getDiyDesignByDiyId($data['customer_creation_id'],1);
        ...
        public function createdesign() {
		    ...
		    if (isset($this->request->post['selected_images'])) {
		        $this->load->model('catalog/directory');
                $selected_images = $this->request->post['selected_images'];
                $selected_images = $this->model_catalog_directory->getImages($selected_images);

                $t = new Ktemplate('template.json');
                $t->leaveOnePage();
                $photo = $t->findFirstPhoto();
                $photos = array();
                foreach ($selected_images as $image){
                    ...
                    $image_item = array(
                        ...
                        'online_url' => '//www.icootoo.com/pub_image/'.$name,
                        'online_thumbs' => array(
                            array(
                                'size'=>'120',
                                'url'=>'//www.icootoo.com/pub_image/120x120_'.$name,
                            ),
                            array(
                                'size'=>'960',
                                'url'=>'//www.icootoo.com/pub_image/960x960_'.$name,
                            )
                        )
                    );
                    $t->wrapImageToPhoto($image_item,$photo);
                    $photos[] = $photo;
                }

                $t->replacePhotos($photos);
                $key = $this->create_uuid();
                $key = $key.'.json';
                $file = DIR_UPLOAD.'jsons/'.$key;
                $t->toString();
                $t->write($file);
                $key  = $this->mv_file_to_s3($file,'cootoo/creations/'.$key,'text/plain');
                $json['url'] = $this->url->link('product/design', 'product_id='.$product_id .'&minimum=1&creation_id='.'#/'.$key.'/');
       ...
       private  function create_uuid($prefix = ""){
            ...
            $str2 = md5(uniqid(mt_rand(), true));
            $uuid  = $str.substr($str2,0,8);
