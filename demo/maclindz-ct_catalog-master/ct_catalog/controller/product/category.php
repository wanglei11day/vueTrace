...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if (isset($this->request->get['path'])) {
            ...
            $this->session->data['wxredirect'] =$this->url->link('product/category', 'path=' . $this->request->get['path']. $url);
		...
		if ($category_info) {
			...
			if ($category_info['image']) {
				...
				$data['image'] = $this->model_tool_image->resize($category_info['image']);
			} else {
				$data['thumb'] = '';
				$data['image'] = '';
			}
			...
			foreach ($results as $result) {
				...
				if (!empty($result['image'])) {
					$cat_thumb = $this->model_tool_image->resize($result['image'], $this->config->get('config_image_product_width'), $this->config->get('config_image_product_width')*1.4);
				} else {
					$cat_thumb = $this->model_tool_image->resize('placeholder.png', $this->config->get('config_image_product_width'), $this->config->get('config_image_product_width')*1.4);
				}
