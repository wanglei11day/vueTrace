<?php
class ControllerAccountResetPassword extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$password_reset = $this->model_account_password_reset->getPasswordByToken($token);
			$customer_id=$password_reset['customer_id'];
			...
			if (!empty($this->request->get['token'])&&!empty($this->request->get['email'])) {
				...
				if(!$this->error){
					...
					$data['text_modify_password'] = $this->language->get('text_modify_password');
