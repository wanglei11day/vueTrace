...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
        ...
        $data['success_redirect'] = $this->url->link('account/account', '', 'SSL');
		...
		if (isset($this->error['verify_code'])) {
			$data['error_verify_code'] = $this->error['verify_code'];
		} else {
			$data['error_verify_code'] = '';
		}
        ...
        if (isset($this->request->post['verify_code'])) {
            $data['verify_code'] = $this->request->post['verify_code'];
        } else {
            $data['verify_code'] = '';
        }
        ...
        if(isset($this->request->post['email'])) {
            ...
            if (empty($domain)) {
                ...
                if (utf8_strlen(trim($this->request->post['verify_code'])) !=4) {
                    $this->error['verify_code'] = $this->language->get('error_verify_code');
                }else{
                    ...
                    if(!isset($this->error['telephone'])&&$code){
                        ...
                        if($code_phone!=$phone){
                            ...
                            if((time()-strtotime($date_added))>10*60){
                                $this->error['verify_code'] = $this->language->get('error_sns_invalid');
                            }
                        }
                    }
                }
