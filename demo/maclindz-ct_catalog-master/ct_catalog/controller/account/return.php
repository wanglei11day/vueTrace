...
class ControllerAccountReturn extends Controller {
	...
	private function sendReturnMail($return_id){
			$this->load->model('account/return');
			$data = array();
			...
			$data['title'] = $this->language->get('text_return');
			...
			$mail->setSubject($this->language->get('heading_title'));
			$mail->setHtml($this->load->view($this->config->get('config_template') . '/template/mail/return.tpl', $data));
			$mail->send();
	}
