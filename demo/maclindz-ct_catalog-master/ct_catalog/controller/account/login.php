...
class ControllerAccountLogin extends Controller {
	...
	public function index() {
        if (!$this->customer->isLogged()&&$this->session->data['weixinbrower']) {
            return new Action('weixinlogin/index');
        }
		...
		if (!empty($this->request->get['token'])) {
			...
			if ($customer_info && $this->customer->login($customer_info['email'], '', true)) {
				...
				if($this->session->data['force_redirect']){
					$this->response->redirect($this->session->data['force_redirect']);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			// Added strpos check to pass McAfee PCI compliance test (http://forum.icootoo.com/viewtopic.php?f=10&t=12043&p=151494#p151295)
