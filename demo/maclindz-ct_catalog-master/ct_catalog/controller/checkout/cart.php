...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$data['text_designfile'] = $this->language->get('text_designfile');
			$data['text_designfile_no'] = $this->language->get('text_designfile_no');
            ...
            $default_design_name = $this->language->get('text_default_design_name');
			...
			foreach ($products as $product) {
				...
				foreach ($product['option'] as $option) {
					...
					$lv =  (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value);
					if(isset($option['onetimefee'])){
						$data['one_time_fee'] = true;

						if($option['price_prefix']&&$option['price']){
							$v = (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value).$this->language->get('text_left_kuohao').$option['price_prefix'].$option['price'].$this->language->get('text_right_kuohao');
						}else{
							$v = $this->language->get('text_left_kuohao').(utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value).$this->language->get('text_right_kuohao');
						}
					$option_data[] = array(
						'name'  => $option['name'],
							'value' => $v,
				...
				$data['products'][] = array(
					...
					'diydesign_name'      => $diydesign?$diydesign['name']?$diydesign['name']:$default_design_name:$default_design_name,
	...
	public function add() {
		...
		if (isset($this->request->post['customer_creation_id'])) {
			...
			$options =  $this->model_account_diy_design->getDesignOptions($customer_creation_id);
			$egnore = true;
			if($options)
		...
		if ($product_info) {
			...
			if(!$egnore&&isset($this->request->post['group_id'])){
			...
			if (!$json) {
				$this->cart->add($this->request->post['product_id'], $quantity, $option, $recurring_id,$customer_creation_id);
				$this->load->model('account/diy_design');
				$this->model_account_diy_design->updateDesignVisiable($customer_creation_id,0);
				...
				$json['total_qty'] = $this->cart->countProducts();
	...
	public function design() {
		...
		if ($product_info) {
			...
			if (!$json) {
				$json['redirect'] = str_replace('&amp;', '&', $this->url->link('product/design', 'product_id=' . $this->request->post['product_id'].'&minimum=1'));
