...
class ControllerTotalCoupon extends Controller {
	...
	public function coupon() {
        ...
        $status = 1;
		$coupon_info = $this->model_total_coupon->getCoupon($coupon,$status);

		if (empty($this->request->post['coupon'])) {
			$json['error'] = "#$status:".$this->language->get('error_empty');
			...
			$json['error'] = "#$status:".$this->language->get('error_coupon');
		}
