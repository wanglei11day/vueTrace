...
class ControllerCommonKFileManager extends Controller {
		...
		foreach ($images as $image) {
			...
			if ($image['type']) {  //如果是目录
				...
				$data['images'][] = array(
					...
					'name'  => $filename,
                    'modify_action' => "<a class=\"modify_action\" title=\"".$this->language->get('text_modify_name')."\" onClick=\"modifyName('".$image['directory_id']."');return false;\" href=".$this->url->link('common/k_filemanager/modify_name', 'path_id='.$image['directory_id'])."><i class=\"fa design-item-icon fa-pencil\"></i></a>" ,
	...
	'dir_name'=>$directory['path'],
					...
					'href'  => $this->url->link('common/k_filemanager', 'token=' . $this->session->data['token'] . '&directory=' . $parent_path.'_'.$image['directory_id'] . $url, 'SSL')
				);
			} else {
...
'modify_action' => "<a class=\"modify_action\" title=\"".$this->language->get('text_modify_name')."\" onClick=\"modifyName('".$image['directory_id']."');return false;\" href=".$this->url->link('common/k_filemanager/modify_name', 'path_id='.$image['directory_id'])."><i class=\"fa design-item-icon fa-pencil\"></i></a>" ,
 ...
 'dir_name'=>$directory['path'],
        ...
        $data['alert_empty_image'] = $this->language->get('alert_empty_image');
        $data['text_button_select'] = $this->language->get('text_button_select');
...
$data['share'] = $share;
		...
		$data['parent'] = $this->url->link('common/k_filemanager', 'token=' . $this->session->data['token'] . $url, '');
		...
		$data['refresh'] = $this->url->link('common/k_filemanager', 'token=' . $this->session->data['token'] . $url, '');
		...
		$pagination->url = $this->url->link('common/k_filemanager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$data['file_breadcrumbs'][] = array(
					...
					'href'  => $this->url->link('common/k_filemanager' . '&init=1'  . $url, '')
		);
		...
		if($share)
        {
            $data['file_breadcrumbs'][] = array(
                ...
                'href'  => $this->url->link('common/k_filemanager' . '&directory=0&share=1'  . $url, '')
            );
        }
        else
        {
            if(!$init)
            {
                $data['file_breadcrumbs'][] = array(
                    ...
                    'href'  => $this->url->link('common/k_filemanager' . '&directory=0'  . $url, '')
		);
            }
		...
		if(!empty($file_path_arr)){
			...
			foreach ($file_path_arr as $dir_id) {
				...
				$data['file_breadcrumbs'][] = array(
					'text' => $filename,
                        'href'  => $this->url->link('common/k_filemanager', '&share=1&directory=' . urlencode($cur_path) . $url, 'SSL')
                    );
                }
                else
                {
                    $data['file_breadcrumbs'][] = array(
                        'text' => $filename,
                        'href'  => $this->url->link('common/k_filemanager', 'directory=' . urlencode($cur_path) . $url, 'SSL')
				);
                }
			...
			$data['file_breadcrumbs'][] = array(
					'text' => $filename,
					'href'  => $this->url->link('common/k_filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode($cur_path) . $url, 'SSL')
				);
		}

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/k_filemanager.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/common/k_filemanager.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/common/k_filemanager.tpl', $data));
        }
	}
	...
	private function getDefaultDir()
    {
        $dirs[] = array(
            ...
            'date_added' =>'',
            'href'  => $this->url->link('common/k_filemanager', 'directory=0&share=1', 'SSL')
        );
        ...
        $dirs[] = array(
            ...
            'date_added' =>'',
            'href'  => $this->url->link('common/k_filemanager', 'directory=0', 'SSL')
        );
        return $dirs;
    }
