...
class ControllerCommonTFileManager extends Controller {
	...
	public function index()
    {
        ...
        if ($init) {
            ...
            foreach ($images as $image) {
                ...
                if ($image['type']) {  //如果是目录
                    ...
                    $data['images'][] = array(
                        'thumb' => '',
                        'name' => $filename,
                        'modify_action' => "<a class=\"modify_action\" title=\"" . $this->language->get('text_modify_name') . "\" onClick=\"modifyName('" . $image['directory_id'] . "');return false;\" href=" . $this->url->link('common/t_filemanager/modify_name', 'path_id=' . $image['directory_id']) . "><i class=\"fa design-item-icon fa-pencil\"></i></a>",

                        'type' => 'directory',
                        'path' => $image['directory_id'],
                        'dir_name' => $directory['path'],
                        'date_added' => date('Y-m-d', strtotime($image['date_added'])),
                        'href' => $this->url->link('common/t_filemanager', 'directory=' . $parent_path . '_' . $image['directory_id'] . $url, '')
                    );
                } else {
            ...
            $data['max_width'] = $this->session->data['t_max_width'];
            $data['max_height'] = $this->session->data['t_max_height'];
            $data['min_width'] = $this->session->data['t_min_width'] ? $this->session->data['t_min_width'] : 1;
            $data['min_height'] = $this->session->data['t_min_height'] ? $this->session->data['t_min_height'] : 1;
            $data['error_size_limit'] = $this->language->get('error_size_limit');
            ...
            $data['parent'] = $this->url->link('common/t_filemanager', 'token=' . $this->session->data['token'] . $url, '');
            ...
            $data['refresh'] = $this->url->link('common/t_filemanager', 'token=' . $this->session->data['token'] . $url, '');
            ...
            $pagination->url = $this->url->link('common/t_filemanager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
            ...
            $data['file_breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('common/t_filemanager' . '&init=1' . $url, '')
            );

            if ($share) {
                $data['file_breadcrumbs'][] = array(
                    'text' => $this->language->get('text_share_dir'),
                    'href' => $this->url->link('common/t_filemanager' . '&directory=0&share=1' . $url, '')
                );
            } else {
                if (!$init) {
                    $data['file_breadcrumbs'][] = array(
                        'text' => $this->language->get('text_my_dir'),
                        'href' => $this->url->link('common/t_filemanager' . '&directory=0' . $url, '')
                    );
                }
            ...
            if (!empty($file_path_arr)) {
                ...
                foreach ($file_path_arr as $dir_id) {
                    ...
                    if ($share) {
                        $data['file_breadcrumbs'][] = array(
                            'text' => $filename,
                            'href' => $this->url->link('common/t_filemanager', '&share=1&directory=' . urlencode($cur_path) . $url, 'SSL')
                        );
                    } else {
                        $data['file_breadcrumbs'][] = array(
                            'text' => $filename,
                            'href' => $this->url->link('common/t_filemanager', 'directory=' . urlencode($cur_path) . $url, 'SSL')
                        );
                    }
                ...
                $data['file_breadcrumbs'][] = array(
                    'text' => $filename,
                    'href' => $this->url->link('common/t_filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode($cur_path) . $url, 'SSL')
                );
            }

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/t_filemanager.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/common/t_filemanager.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/common/t_filemanager.tpl', $data));
            }
        }
    }
	...
	private function getDefaultDir()
    {
        $dirs[] = array(
            ...
            'date_added' =>'',
            'href'  => $this->url->link('common/t_filemanager', 'directory=0&share=1', 'SSL')
        );
        ...
        $dirs[] = array(
            ...
            'date_added' =>'',
            'href'  => $this->url->link('common/t_filemanager', 'directory=0', 'SSL')
        );
        return $dirs;
    }
	            ...
	            if ( !empty($file_ary) ) {
	                ...
	                foreach($file_ary as $cur_file) {
	                    ...
	                    if ($this->model_catalog_directory->checkExist($directory,$filename)) {
						    ...
						    $upload_status = $this->mv_file_to_s3($cur_file['tmp_name'] , $filename,$cur_file['type']);  //上传到亚马�?
		                    ...
		                    $upload_status = $this->mv_file_to_s3($new_filename , $thumb_name,$cur_file['type']);  //上传缩略图到亚马�?
