...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
        ...
        if(isset($data['email'])){
            ...
            $domain = strstr($data['email'],'@');
            if(empty($domain)) {
                $data['telephone'] = $data['email'];
                $data['email']='fake_'.$data['telephone'].'_'.time().'@icootoo.com';
                $data['no_email']=1;
            }
        }else{
            $data['no_email']=0;
    ...
    public function editPasswordByPhone($telephone, $password) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($password)))) . "' WHERE LOWER(telephone) = '" . $this->db->escape(utf8_strtolower($telephone)) . "'");
    ...
    public function getCustomersByInfo($info) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE (LOWER(email) = '" . $this->db->escape(utf8_strtolower($info)) . "' OR telephone = '".$this->db->escape(utf8_strtolower($info))."')");
