...
class ModelTotalCoupon extends Model {
	public function getCoupon($code,&$s = '0') {
		$status = true;
		$logger = new Log("coupon.log");
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1'");
		if ($coupon_query->num_rows) {
		    $sub_total = $this->cart->getSubTotal();
			if ($coupon_query->row['total'] > $sub_total) {
                $logger->write("subtotal ==========$sub_total".PHP_EOL);
				$logger->write("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE code = '" . $this->db->escape($code) . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1'");
				$status = false;
				$s = 2;
			}
			...
			if ($coupon_query->row['uses_total'] > 0 && ($coupon_history_query->row['total'] >= $coupon_query->row['uses_total'])) {
				$logger->write("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "coupon_history` ch WHERE ch.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "'");
                $s = 3;
				$status = false;
			}
			...
			if ($this->customer->getId()) {
				...
				if ($coupon_query->row['uses_customer'] > 0 && ($coupon_history_query->row['total'] >= $coupon_query->row['uses_customer'])) {
					$logger->write("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "coupon_history` ch WHERE ch.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "' AND ch.customer_id = '" . (int)$this->customer->getId() . "'");
                    $s = 5;
					$status = false;
				}
			}
			...
			if ($coupon_product_data || $coupon_category_data) {
				foreach ($this->cart->getProducts() as $product) {
					...
					foreach ($coupon_category_data as $category_id) {
                        $logger->write("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "product_to_category` WHERE `product_id` = '" . (int)$product['product_id'] . "' AND category_id = '" . (int)$category_id . "'");
				...
				if (!$product_data) {
					$logger->write("SELECT * FROM `" . DB_PREFIX . "coupon_product` WHERE coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "'");
                    $s = 6;
					$status = false;
				}
			}
	...
	public function confirm($order_info, $order_total) {
		$code = '';
		$logger = new Log("coupon.log");
		$logger->write("coupon confirm!!!!!!!");
		$start = strpos($order_total['title'], '(') + 1;
		$end = strrpos($order_total['title'], ')');
        $logger->write(print_r($order_total,true));
		if ($start && $end) {
			$code = substr($order_total['title'], $start, $end - $start);
		}

		$logger->write("coupon $code!!!!!!!");
		if ($code) {
			$coupon_info = $this->getCoupon($code);

			if ($coupon_info) {
				$logger->write("INSERT INTO `" . DB_PREFIX . "coupon_history` SET coupon_id = '" . (int)$coupon_info['coupon_id'] . "', order_id = '" . (int)$order_info['order_id'] . "', customer_id = '" . (int)$order_info['customer_id'] . "', amount = '" . (float)$order_total['value'] . "', date_added = NOW()");
