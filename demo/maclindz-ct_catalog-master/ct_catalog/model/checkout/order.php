...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if ($product['customer_creation_id']) {
					$diydesignquery = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_design WHERE customer_creation_id= '".$product['customer_creation_id']."'");    
					$add_price = $diydesignquery->row['add_price'];
				}
				$onetimefee = isset($product['onetimefee'])?$product['onetimefee']:0;
				
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', onetimefee = '" . (float)$onetimefee. "', add_price = '" . (float)$add_price. "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', customer_creation_id = '" . $this->db->escape($product['customer_creation_id']) . "'");
		...
		if (isset($data['vouchers'])) {
			foreach ($data['vouchers'] as $voucher) {
				...
				$voucher_id = $this->model_total_voucher->addVoucher(0, $voucher);

				$this->db->query("UPDATE " . DB_PREFIX . "order_voucher SET voucher_id = '" . (int)$voucher_id . "' WHERE order_voucher_id = '" . (int)$order_voucher_id . "'");
			}
		}

		if(isset($this->session->data['hurryvalue'])&&$this->session->data['hurryvalue']){
            $this->db->query("UPDATE oc_order set hurry = '".$this->session->data['hurryvalue']."' WHERE order_id = '".$order_id."'");
        }
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		$logger = new Log('add_order_history.log');
		$logger->write("begin:---------------------------\n");
		...
		if ($order_info) {
			...
			$logger->write(".................".PHP_EOL);
            $logger->write('last order_status id----'.$order_id.':'.$order_status_id."\n");
