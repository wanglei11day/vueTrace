<?php
/**
 * $Author: http://www.opencartchina.com 
**/
// Heading
$_['heading_title']                  = '结账';

// Text
$_['text_cart']                      = '购物车';
$_['text_checkout_option']           = '第 1 步： 结账选项';
$_['text_checkout_hurry']  = '第 2 步： 加急服务';
$_['text_checkout_amount'] = 'Step 2: 充值金额';
$_['text_checkout_account']          = 'Step 2: Account &amp; Billing Details';
$_['text_checkout_shipping_address'] = '第 3 步： 配送地址';
$_['text_checkout_shipping_method']  = '第 4 步： 配送方式';
$_['text_checkout_payment_method']   = '第 5 步： 支付方式';
$_['text_checkout_confirm']          = '第 6 步： 确认订单';
$_['text_modify']                    = '更新 &raquo;';
$_['text_new_customer']              = '新会员注册';
$_['text_returning_customer']        = '注册会员';
$_['text_checkout']                  = '结账选项:';
$_['text_i_am_returning_customer']   = '我已注册账户';
$_['text_register']                  = '注册账户';
$_['text_guest']                     = '游客结账';
$_['text_register_account']          = '注册一个账户以便您更快捷地购物， 查看订单状态， 查看采购的历史记录， 和更多的管理项目。';
$_['text_forgotten']                 = '忘记密码';
$_['text_your_details']              = '个人资料';
$_['text_your_address']              = '地址';
$_['text_your_password']             = '密码';
$_['text_agree']                     = '已阅读并同意 <a href="%s" class="agree"><b>%s</b></a>';
$_['text_address_new']               = '添加一个新地址';
$_['text_tip_rush_service']               = '请联系客服';
$_['text_address_existing']          = '使用现存地址';
$_['text_shipping_method']           = '请选择配送方式。';
$_['text_payment_method']            = '请选择支付方式。';
$_['text_comments']                  = '备注';
$_['text_recurring_item']            = '分期付款商品';
$_['text_payment_recurring']         = '分期付款方式';
$_['text_trial_description']         = '%s every %d %s(s) for %d payment(s) then';
$_['text_payment_description']       = '%s every %d %s(s) for %d payment(s)';
$_['text_payment_cancel']            = '%s every %d %s(s) until canceled';
$_['text_day']                       = '天';
$_['text_hour']                       = '小时';
$_['text_week']                      = '周';
$_['text_semi_month']                = '半月';
$_['text_month']                     = '月';
$_['text_year']                      = '年';
$_['text_hurry']                      = '<b> - %s %s 加急:</b>在原价上增加 %s%% 或最低  %s;';
$_['text_hurry_tip']                      = '订单确认后制作周期为5个工作日，PVC相册制作周期为14天。';
$_['text_exchange_charge']                      = '手续费';
$_['text_need_fapiao'] = '是否需要发票?';
$_['text_fapiao_title'] = '发票抬头: ';
// Column
$_['column_name']                    = '商品名称';
$_['column_model']                   = '型号';
$_['column_quantity']                = '数量';
$_['column_price']                   = '单价';
$_['column_total']                   = '小计';

// Entry
$_['entry_email_address']            = 'E-Mail 地址';
$_['entry_email']                    = 'E-Mail';
$_['entry_password']                 = '密码';
$_['entry_confirm']                  = '确认密码';
$_['entry_firstname']                = '收件人姓名';
$_['entry_lastname']                 = '姓氏';
$_['entry_telephone']                = '联系电话';
$_['entry_fax']                      = '传真';
$_['entry_address']                  = '选择地址';
$_['entry_company']                  = '公司';
$_['entry_customer_group']           = '会员等级';
$_['entry_address_1']                = '地址';
$_['entry_address_2']                = '地址 2';
$_['entry_postcode']                 = '邮政编码';
$_['entry_city']                     = '所在城市';
$_['entry_country']                  = '国家';
$_['entry_zone']                     = '州/省/地区';
$_['entry_newsletter']               = '订阅 %s 新闻邮件。';
$_['entry_shipping'] 	             = '配送地址与账单地址一致。';
$_['entry_deposit_amount'] = '金额:';
// Error
$_['error_warning']                  = '存在问题，无法完成订单！如果持续出现此问题，请变换付款方式或者 <a href="%s">点击这里</a>联系网站管理员。';
$_['error_login']                    = '警告: 用户名或密码错误。';
$_['error_attempts']                 = '警告: 超过登录失败最大次数，请一小时后再登录。';
$_['error_approved']                 = '警告: 登录前需要审核通过。';
$_['error_exists']                   = '警告: E-Mail 已被注册使用！';
$_['error_firstname']                = '名称必须为 1 - 32字符！';
$_['error_lastname']                 = '姓氏必须为 1 - 32字符！';
$_['error_email']                    = 'E-Mail 无效！';
$_['error_telephone']                = '电话必须为 3 - 32字符！';
$_['error_password']                 = '密码必须为 3 - 20字符！';
$_['error_confirm']                  = '确认密码与密码不一致！';
$_['error_address_1']                = '地址 1必须为 3 - 128字符！';
$_['error_city']                     = '所在城市必须为 2 - 128字符！';
$_['error_postcode']                 = '邮政编码必须为 2 - 10字符！';
$_['error_country']                  = '请选择国家';
$_['error_zone']                     = '请选择州/省/地区！';
$_['error_agree']                    = '警告:必须同意 %s!';
$_['error_address']                  = '警告: 必须选择地址！';
$_['error_shipping']                 = '警告: 必须选择配送方式！';
$_['error_no_shipping']              = '警告: 无配送选项，请 <a href="%s">联系我们</a> 获取帮助！';
$_['error_payment']                  = '警告: 必须选择支付方式！';
$_['error_no_payment']               = '警告: 无支付选项，请 <a href="%s">联系我们</a> 获取帮助！';
$_['error_custom_field']             = '%s 必须！';
